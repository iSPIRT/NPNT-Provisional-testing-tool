...
class AppWindow(QDialog):
    def __init__(self):
        ...
        self.create_verify_results_button()
    ...
    def create_permission_gen_btn(self):
        ...
        self.generate_permission_buttonwidget.resize(self.generate_permission_buttonwidget.sizeHint())

        layout = QHBoxLayout()
        self.generate_tests_button = QPushButton("Generate Test cases")
        self.generate_tests_button.clicked.connect(self.check_inputs_and_create_tests)
    ...
    def create_permission_tests_window(self):
        ...
        download_icon = QtGui.QPixmap('download_1.png')
        ...
        pa_1_download_button.clicked.connect(lambda : self.save_permission_artefact(0))

        self.pass_1 = QRadioButton("Yes")
        self.fail_1 = QRadioButton("No")
        self.pass_1.toggled.connect(lambda: self.update_responses(self.pass_1,0))
        self.fail_1.toggled.connect(lambda: self.update_responses(self.fail_1,0))
        ...
        pa_2_download_button.clicked.connect(lambda : self.save_permission_artefact(1))

        self.pass_2 = QRadioButton("Yes")
        self.fail_2 = QRadioButton("No")
        self.pass_2.toggled.connect(lambda: self.update_responses(self.pass_2,1))
        self.fail_2.toggled.connect(lambda: self.update_responses(self.fail_2,1))
        ...
        pa_3_download_button.clicked.connect(lambda : self.save_permission_artefact(2))

        # TODO -  rename all the pass/fail to Armed/ Disarmed
        self.pass_3 = QRadioButton("Yes")
        self.fail_3 = QRadioButton("No")
        self.pass_3.toggled.connect(lambda: self.update_responses(self.pass_3,2))
        self.fail_3.toggled.connect(lambda: self.update_responses(self.fail_3,2))
        ...
        pa_4_download_button.clicked.connect(lambda : self.save_permission_artefact(3))

        self.pass_4 = QRadioButton("Yes")
        self.fail_4 = QRadioButton("No")
        self.pass_4.toggled.connect(lambda: self.update_responses(self.pass_4,3))
        self.fail_4.toggled.connect(lambda: self.update_responses(self.fail_4,3))
        ...
        pa_5_download_button.clicked.connect(lambda : self.save_permission_artefact(4))

        self.pass_5 = QRadioButton("Yes")
        self.fail_5 = QRadioButton("No")
        self.pass_5.toggled.connect(lambda: self.update_responses(self.pass_5,4))
        self.fail_5.toggled.connect(lambda: self.update_responses(self.fail_5,4))
        ...
        row_6_download_button.clicked.connect(lambda : self.save_permission_artefact(valid_pa = True))
    ...
    def save_permission_artefact(self, index = None, valid_pa = False):
        if valid_pa:
            index = 5
            current_perm_xml = generate_valid_permission(self.drone_id_entry.text() , self.drone_public_key)
        else:
            current_perm_xml = self.permission_cases[index]
        folder = str(QFileDialog.getExistingDirectory(self, "Select Directory"))
        save_path = os.path.join(folder, 'signed_permission_artefact_{}.xml'.format(index+1))
    ...
    def update_responses(self, btn, index):
        if btn.text() == "Yes":
            if btn.isChecked() == True:
                self.responses[index] = True
            else:
                self.responses[index] = False
        if btn.text() == "No":
            if btn.isChecked() == True:
                self.responses[index] = False
            else:
    ...
    def create_verify_results_button(self):

        self.verify_results_buttonwidget = QWidget()
        self.verify_results_buttonwidget.resize(self.generate_permission_buttonwidget.sizeHint())

        layout = QHBoxLayout()
        verify_results_button = QPushButton("Verify Results")
        verify_results_button.setToolTip("Click to verify the testing results to check if the application clears or fails.")
    ...
    def check_and_verify_results(self):
        ...
        try:
            ...
            self.flight_log_sign_verified = verify_flight_log_authenticity(log, self.drone_public_key)
        except Exception as e:
            self.show_warning("Incorrect file format. Signature verification failed/n"
        ...
        if tests_passed:
            ...
            response_results = ["Passed" if (x == y) else "Failed - Incorrect Response" for x, y in zip(self.permission_case_truth, self.responses)]
            test_nums = ["Test {} result : ".format(x) for x in range(1, 6)]
            response_text = "\n".join(x+y for x,y in zip(test_nums,response_results))
            self.show_failure_dialog("Responses did not match.\n\n{}".format(response_text))
    ...
    def setup_test_scenarios(self):
        ...
        cases = generate_all_test_permission_artefacts(self.drone_id_entry.text() , self.drone_public_key)
        shuffle(cases)
    ...
    def drone_key_browser(self):
        proceed = True
        drone_key_input, _filter = QFileDialog.getOpenFileName(self, "Select RSA 2048 public key of RPAS", filter='*.pem')
    ...
    def upload_log_browser(self):
        self.log_file, _filter = QFileDialog.getOpenFileName(self, "Select test flight log" )
    ...
    def verify_keyin(self, public_key_path):
        ...
        """
        if not os.path.exists(public_key_path) or not public_key_path.lower().endswith('.pem'):
            return False
        else:
